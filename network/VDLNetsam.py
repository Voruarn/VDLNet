...
from network.ModulesSAM import ConvNextModel, MutliScaleFusion, TextEncoder
from network.SODmaskdecoder import SODMaskDecoder

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

class VDLNetsam(nn.Module):
    ...
    def forward(self, rgb, depth, texts,):
        ...
        visual_feats = visual_depth_feats['visual']  # [V1,C1; V2,C2; V3,C3; V4,C4]
        depth_feats = visual_depth_feats['depth']    # [D1,C1; D2,C2; D3,C3; D4,C4]
        
        # Step 2: U型多尺度融合（到C1层）
        fused_rgb_c1 = self.rgb_msf(visual_feats)    # (B,256,H/4,W/4)
        fused_depth_c1 = self.depth_msf(depth_feats) # (B,256,H/4,W/4)
        
        # Step 3: SAM Decoder前向

...
class SODMaskDecoder(nn.Module):
    ...
    def __init__(
        self,
        *,
        input_feat_dim: int = 256,       # VDLNet的dec_dim
        ...
        # 1. 稠密提示投影（深度特征→transformer维度，适配VDLNet的dense_prompt_feat）
        self.dense_prompt_proj = nn.Conv2d(input_feat_dim, transformer_dim, kernel_size=1)
        ...
        # 6. IoU预测头（可选，显著目标检测可保留）
        self.iou_prediction_head = MLP(
            transformer_dim, iou_head_hidden_dim, 1 + num_multimask_outputs, iou_head_depth
        )
    ...
    def forward(
        ...
        image_embedding = image_embedding + dense_prompt  # 深度作为稠密提示叠加
